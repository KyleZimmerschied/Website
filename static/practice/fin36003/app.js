(() => {
  ...
  const state = {topics:new Set(bank.topics.map(t=>t.id)),types:new Set(["mc","quant"]),diffs:new Set(),count:20,session:[],timer:null,seconds:0,submitted:false};
  ...
  function startSession(preset=false){
    ...
    if(preset){
      ...
      const quant=shuffle(bank.questions.filter(q=>q.kind==="quant"&&["m","h"].includes(q.diff))).slice(0,4);
      selected=[...mc,...quant];state.count=14;
    } else selected=chooseQuestions(eligible(),state.count);
    if(!selected.length)return;
    state.session=selected;state.submitted=false;state.seconds=selected.length*120;
    selected.forEach(q=>seen.add(q.id));localStorage.setItem("finn36003-seen",JSON.stringify([...seen]));
    $("#session-title").textContent=preset?"Exam 1-style practice":"Custom practice set";$("#session-meta").textContent=`${selected.length} questions · ${selected.filter(q=>q.kind==="mc").length} multiple choice · ${selected.filter(q=>q.kind==="quant").length} quantitative`;
    $("#results").hidden=true;$("#submit-session").hidden=false;renderSession();showView("session");
    clearInterval(state.timer);const timer=$("#timer");timer.hidden=!$("#timed").checked;
  ...
  $("#timed").onchange=updateSummary;$("#generate").onclick=()=>startSession(false);$("#exam-preset").onclick=()=>startSession(true);$("#submit-session").onclick=gradeSession;$("#leave-session").onclick=()=>{clearInterval(state.timer);showView("practice");};
  ...
  $("#bank-count").textContent=bank.questions.length;renderTopics();renderPills();updateSummary();setupBrowse();renderFormulas();
})();
